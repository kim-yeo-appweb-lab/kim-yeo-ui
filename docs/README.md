# 문서

@kim-yeo/ui 디자인 시스템 문서 모음입니다.

## 목차

### 시작하기

- [README](../README.md) - 프로젝트 소개 및 설치 가이드
- [기여 가이드](../CONTRIBUTING.md) - 프로젝트에 기여하는 방법

### 개발 가이드

- [배포 가이드](DEPLOYMENT.md) - GitHub Actions를 통한 자동 배포 설정
- [디자인 토큰](DESIGN_TOKENS.md) - 3단계 토큰 시스템 및 커스터마이징 가이드

### Foundation (Storybook)

Storybook에서 확인 가능한 디자인 시스템 기초 문서:

- **Introduction** - 프로젝트 소개 및 빠른 시작
- **Colors** - 색상 팔레트 및 토큰 시스템
- **Typography** - 폰트 패밀리, 크기, 무게, 행간
- **Spacing** - 간격 시스템 (Padding, Margin, Gap)
- **Accessibility** - 접근성 가이드 및 WAI-ARIA 표준

### 참고 자료

- [Storybook](https://kim-yeo-appweb-lab.github.io/kim-yeo-ui/) - 컴포넌트 라이브러리 데모
- [GitHub](https://github.com/kim-yeo-appweb-lab/kim-yeo-ui) - 소스 코드 저장소
- [npm](https://www.npmjs.com/package/@kim-yeo/ui) - npm 패키지
- [Issues](https://github.com/kim-yeo-appweb-lab/kim-yeo-ui/issues) - 이슈 트래커
